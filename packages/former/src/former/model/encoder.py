...
from former.model.attention import MultiHeadedAttention
from former.model.attention import StreamingRelPositionMultiHeadedAttention
from former.model.convolution import ConvolutionModule
from former.model.embedding import StreamingRelPositionalEncoding
from former.model.encoder_layer import ChunkFormerEncoderLayer
from former.model.positionwise_feed_forward import PositionwiseFeedForward
from former.model.subsampling import DepthwiseConvSubsampling
from former.model.utils.common import get_activation
from former.model.utils.mask import make_pad_mask
...
class BaseEncoder(torch.nn.Module):
    def __init__(
        ...
        self.limited_decoding_chunk_sizes = torch.IntTensor(
    ...
    def forward_parallel_chunk(
        ...
        xs_lens = []
        n_chunks = []
        for xs_origin_len, x, offs in zip(
            xs_origin_lens, xs, offset
        ):  # cost O(input_batch_size | ccu)
            x = x.to(device)
            if x.size(0) >= size:
        ...
        xs, pos_emb, xs_lens = self.embed(
            xs, xs_lens, offset=left_context_size, right_context_size=right_context_size
        )
        ...
        for i, layer in enumerate(self.encoders):
            xs, _, new_att_cache, new_cnn_cache = layer.forward_parallel_chunk(
                ...
                att_cache=(
                    att_cache[i].to(device) if att_cache.size(0) > 0 else att_cache
                ),
                cnn_cache=(
                    cnn_cache[i].to(device) if cnn_cache.size(0) > 0 else cnn_cache
                ),
    ...
    def rearrange(self, xs, xs_lens, n_chunks):
        ...
        masks = (
            ~make_pad_mask(xs_lens, xs.size(1)).unsqueeze(1).to(xs.device)
        )  # (B, 1, T)
        return xs, masks
